...
const ALLOW_GITHUB_ON_PREVIEW = /^true|1|yes$/i.test(process.env.GITHUB_ALLOW_PREVIEW ?? "")
...
if (ENFORCE_SMTP) {
  ...
  if (
    ...
    console.warn("[NextAuth][Email] SMTP envs incomplete in non-production; Email provider will be omitted.")
  }
}

// ---------- 프로바이더 조건부 등록 ----------
function hasEmailEnv() {
  return Boolean(EMAIL_HOST && process.env.EMAIL_SERVER_PORT && typeof process.env.EMAIL_SERVER_SECURE !== "undefined" && EMAIL_USER && EMAIL_PASS && EMAIL_FROM)
}
function hasGithubEnv() {
  return Boolean(GITHUB_ID && GITHUB_SECRET)
}
function githubAllowedHere() {
  return hasGithubEnv() && (process.env.VERCEL_ENV !== "preview" || ALLOW_GITHUB_ON_PREVIEW)
}
...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    ...
    // ───────── GitHub (프리뷰 기본 비활성, 허용 플래그 있으면 활성) ─────────
    ...(githubAllowedHere()
